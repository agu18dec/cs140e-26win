...
has_data_timeout(unsigned timeout) {
    ...
    while(!uart_has_data()) {
        if ((unsigned)(timer_get_usec() - start) > timeout)
            return 0;
    }
    return 1;
}
...
static void wait_for_data(unsigned usec_timeout) {
    while(1) {
        ...
        if (has_data_timeout(usec_timeout) == 1) return;
    }
}
...
uint32_t get_code(void) {
    ...
    uint32_t boot_start = (uint32_t)(uintptr_t)&PUT32;
    uint32_t boot_end   = (uint32_t)(uintptr_t)&__prog_end__;
    uint32_t end = addr + nbytes;

    if(addr < boot_end && end > boot_start) // overlap check
        boot_err(BOOT_ERROR, "binary would collide with bootloader\n");
    ...
    uint32_t got = crc32((const void*)(uintptr_t)addr, nbytes);
    if(got != cksum)
